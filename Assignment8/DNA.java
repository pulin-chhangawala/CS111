...
public class DNA {
    ...
    public void createDatabaseOfProfiles ( String filename ) {
        ...
        int numProfile = Integer.parseInt(StdIn.readLine());
        database = new Profile[numProfile];
        for(int i = 0; i< numProfile; i++){
            String name = StdIn.readLine();
            String seq1 = StdIn.readLine();
            String seq2 = StdIn.readLine();
            Profile profile = new Profile(name, null, null, seq1, seq2);
            database[i] = profile;
        }
    }
    ...
    public void readSTRsOfInterest ( String filename ) {
        ...
        int numSTRs = Integer.parseInt(StdIn.readLine());
    ...
    StdIn.setFile(filename); // DO NOT remove this line, keep as the first line in the method.
        String seq1 = StdIn.readLine();        
        String seq2 = StdIn.readLine();
        return new Profile("Unknown", null, null, seq1, seq2);
    }
    ...
    public void createProfileSTRs ( Profile profile, String[] allSTRs ) {  
        STR[] seq1Counts = new STR[allSTRs.length];
        STR[] seq2Counts = new STR[allSTRs.length];
        for(int i = 0; i < allSTRs.length; i++){
            STR seq1Result = findSTRInSequence(profile.getSequence1(), allSTRs[i]);
            STR seq2Result = findSTRInSequence(profile.getSequence2(), allSTRs[i]);
            seq1Counts[i] = seq1Result;
            seq2Counts[i] = seq2Result;
        }
        profile.setS1_STRs(seq1Counts);
        profile.setS2_STRs(seq2Counts);
    }
    ...
    public ArrayList<Profile> findMatchingProfiles ( STR[] unknownProfileS1_STRs ) {
        ...
        for(int i = 0; i < database.length; i++){
            if(identicalSTRs(unknownProfileS1_STRs, database[i].getS1_STRs())){
